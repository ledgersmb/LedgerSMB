...
FOR retval IN
       ...
       SELECT g.id, g.type, g.invoice, g.reference, g.vencusname, g.description, ac.transdate,
         ...
         FROM (select id, 'gl' as type, false as invoice, reference,
                      null::text as vencusname, description, approved,
                      null::text as till
                 FROM gl
               UNION
    ...
    LEFT JOIN bu_tree ON bac.bu_id = bu_tree.id
     ...
     GROUP BY g.id, g.type, g.invoice, g.reference, g.vencusname, g.description, ac.transdate,
              ac.source, ac.amount_bc, c.accno, c.gifi_accno,

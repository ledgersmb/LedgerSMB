...
define([
   ...
   if (!window.global[onload]) {
      window.global[onload] = function () {
   ...
   function create(name, onloadstr, uri) {
      ...
      if (window.global.frames[name]) {
         return window.global.frames[name];
      }
      ...
      window.global[name] = frame;

      return frame;
   }

   function setSrc(_iframe, src, replace) {
      var frame = window.global.frames[_iframe.name];
   ...
   function doc(iframeNode) {
      ...
      if (name) {
         ...
         if (window.doc.frames[name] && window.doc.frames[name].document) {
            return window.doc.frames[name].document;
         }
      }
      return null;
   }
         ...
         if (formNode) {
            if (!dfd._legacy) {
               ...
               do {
                  ...
                  parentNode &&
                  parentNode !== window.doc.documentElement
               );
               ...
               if (!parentNode) {
                  ...
                  window.body().appendChild(formNode);
               }

...
sub pre_bulk_post_report {
    ...
    for my $crow (1 .. $request->{contact_count}){
        ...
        if ($request->{"id_$cid"}){
            $ref = {
                pay_to    => $request->{"contact_label_$cid"},
...
sub payment {
    ...
    my @currOptions;
    my @arrayOptions = $Settings->get_currencies();

    for my $ref (0 .. $#arrayOptions) {
        push @currOptions, {
            value => $arrayOptions[$ref],
...
sub payment2 {
    ...
    my @vc_options = $Payment->get_vc_info();
    ...
    if ($default_currency ne $request->{curr} ) {
        ...
        push(
            @column_headers,
            {text => $locale->text('Exchange Rate')},
    ...
    for my $ref (0 .. $#array_options) {
        ...
        if (  !$request->{"checkbox_$array_options[$ref]->{invoice_id}"}) {
            ...
            push @invoice_data, {
                ...
                due => $Payment->format_amount(
                    amount => $request->{"optional_discount_$array_options[$ref]->{invoice_id}"} ? $array_options[$ref]->{due}
    ...
    my $select = {
        ...
        stylesheet => $request->{_user}->{stylesheet},
        header => {
            text => $request->{type} eq 'receipt' ? $locale->text('Receipt') : $locale->text('Payment')
        },
        type => {
            name => 'type',
            value =>  $request->{type}
    ...
    $select->{selected_account} = $vc_options[0]->{cash_account_id} unless defined $select->{selected_account};
    ...
    for my $ref (0 .. $#array_options) {
        if ((!$request->{"checkbox_$array_options[$ref]->{invoice_id}"})&&($request->{"topay_fx_$array_options[$ref]->{invoice_id}"})) {
            ...
            if (
                ($request->{"optional_discount_$array_options[$ref]->{invoice_id}"})
                &&
                ("$array_options[$ref]->{due_fx}" <= $request_topay_fx_bigfloat + $array_options[$ref]->{discount_fx})
            ) {
            ...
            if (
                $request->round_amount($sign * "$array_options[$ref]->{due_fx}")
                <
                ...
                $unhandled_overpayment = $request->round_amount(
                    $unhandled_overpayment +
                    $request_topay_fx_bigfloat +
                    $temporary_discount -
                    $array_options[$ref]->{amount}
                );
            }
...
sub print_payment {
    ...
    for my $row (@rows) {
        $row->{amount} = $Payment->format_amount({
            amount => $row->{amount},
            money => 1
        });
    }
...
sub use_overpayment2 {
    ...
    while ($Payment->{"entity_id_$count"}) {
        ...
        if ($ovp_repeated_invoices{qq|$Payment->{"invoice_id_$count"}|}->{qq|$Payment->{"selected_accno_$count"}|} != $Payment->{"selected_accno_$count"}){
            ...
            my $applied_due =
                ($ovp_repeated_invoices{qq|$Payment->{"invoice_id_$count"}|}->{"optional_discount"}) ? $Payment->{"due_$count"}
            ...
            if($invoice_id_amount_to_pay{qq|$Payment->{"invoice_id_$count"}|} > $applied_due){
                $warning .= $locale->text(
                    'The amount of the invoice number').qq| $Payment->{"invnumber_$count"} |.$locale->text('is lesser than the amount to be paid'
                ).qq|\n|;
            }
	...
	datepaid => {
        ...
        warning => $warning,
        header => {
            text => $locale->text('Use overpayment/prepayment')
        },
    };
...
sub post_overpayment {
    ...
    while ($request->{"entity_id_$count"}) {
        ...
        # Warnings Section
        ###############################################################
        #In this section, the post_overpayment will check some user inputs and verify
        #if those are apted to call the post method, if not just store a warning message in the
        #$request->{warning} variable and then call the use_overpayment2 method and it will manage it

        #the $invoice_id_amount_to_pay hash will keep track of the amount to be paid of an
        #specific invoice_id, this amount could not be more than the due of that invoice
        ...
        if($entity_list{"$entity_id"}) {
            ...
            if (
                $entity_list{"$entity_id"}->{"optional_discount_$count"} &&
                $entity_list{"$entity_id"}->{"amount_$count"} == $entity_list{"$entity_id"}->{"due_$count"}
            ) {
            ...
            while($tmp_ovp_amount > 0) {
                ...
                if (@{$entity_unused_ovp{"$ovp_chart_id"}->{"unused_overpayment"}}[$unused_ovp_index]->{"available"} eq '') {
                    $request->{"warning"} .= $locale->text('The amount to be pay from the accno').
                                             qq| $ovp_chart_id |.
                                             $locale->text('is bigger than the amount available').
                                             qq|\n|;
                    $tmp_ovp_amount = -1;
                    next;
                }

...
LedgerSMB is a free and interactive web-based double-entry accounting system, featuring
quotations, ordering, invoicing, projects, timecards, inventory management,
shipping, and more...

Directly send orders and invoices from the built-in e-mail function to your
customers or RFQs (Request For Quotation) to your vendors, complete with PDF attachments,
from anywhere in the world with an easy-to-use browser-based UI.

With its data stored in the enterprise-strength `PostgreSQL` open source relational
...
fast customization.  Supported output formats include PDF, CSV, HTML, ODF, and more.
...
The web server is only required for production installs; for evaluation purposes
simpler setup can be used, as detailed below.

## Client

The tables hereunder show the browsers currently supported, their earliest dates,
...
a regular HTTP connection is unsupported by the project team.  Please be aware
that using HTTPS is the recommended setup, so that Safari is considered to be
fully supported.

**Note**: Earliest dates and versions are sourced from [caniuse.com](`https://www.caniuse.com/`).
Only the most recent data is available for Mobile browsers other than Safari,
because they are pushed out to the devices as soon as they are released, so the
number of devices running older web browsers is negligible.
...
because it lacks secure connections that protect your users' passwords.
...
**NOTE:** This setup does not use a web server like nginx or Apache.  Setups
that do include one will yield a faster user experience due to (much) faster
page load times and web request responses.  For production setups, please
consider adding a web server to the installation.

...
|;
    ...
    <tr>
      ...
      <td><input data-dojo-type="dijit/form/TextBox" name="customerprice_$i" size=10 value=|
          . $form->format_amount( \%myconfig, $form->{"customerprice_$i"} )
          . qq|></td>
      $currency
...
|;
    ...
    foreach my $i ( 1 .. $numrows ) {
        ...
        $linetotalsellprice =
          $form->format_amount( \%myconfig, $linetotalsellprice );
        $linetotallistprice =
          $form->format_amount( \%myconfig, $linetotallistprice );
        $linetotallastcost =
          $form->format_amount( \%myconfig, $linetotallastcost );
...
|;
    ...
    $column_data{sellprice} =
      "<th align=right>"
      . $form->format_amount( \%myconfig, $form->{sellprice} ) . "</th>";
    $column_data{listprice} =
      "<th align=right>"
      . $form->format_amount( \%myconfig, $form->{listprice} ) . "</th>";
    $column_data{lastcost} =
      "<th align=right>"
      . $form->format_amount( \%myconfig, $form->{lastcost} ) . "</th>";
